...
export async function extractFromText(text, model, existingEntities) {
...
export async function resolveEntityConflicts(newEntity, existingEntity, model) {
    const startTime = performance.now();
...
export async function detectContradictions(newFact, existingFacts, model) {
...
export async function detectContradictionsBatch(newFacts, existingFacts, model) {
