...
export async function generateChunkContext(chunk, model = google('gemini-2.0-flash')) {
...
export async function generateContextBatch(chunks, options = {}) {
...
function buildContextualContent(chunk, context) {
    const header = [
        `File: ${chunk.filePath}`,
...
function buildFallbackContext(chunk) {
    const parts = [];
...
export function createContextualChunkWithoutLLM(chunk) {
