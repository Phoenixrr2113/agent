...
import { generateContextBatch, createContextualChunkWithoutLLM, } from './context.js';
import { createBM25Index, mergeSearchResults } from './bm25.js';
import { rerankWithFallback } from './rerank.js';
import { createDefaultRegistry } from './strategies/index.js';
import { logger } from '@agent/shared';
export function createCodebaseRAG(workspaceRoot, options = {}) {
    const { enableCache = true, enableContextGeneration = true, enableBM25 = true, enableReranking = true, rerankTopN = 100, returnTopN = 20, onProgress, strategyRegistry, } = options;
    const registry = strategyRegistry ?? createDefaultRegistry();
    let embeddedChunks = [];
    let bm25Index = null;
    const chunkMap = new Map();
    const cache = createFileCache(path.join(workspaceRoot, '.rag-cache'));
    const log = (message) => {
        onProgress?.(message);
    };
    ...
    const scanWorkspaceFallback = async () => {
        const chunks = [];
