...
export const startAgentTaskTool = tool({
  description: 'Start an autonomous agent session as a background task. The agent will work on the given task independently, using all available tools. Perfect for: complex research, multi-step builds, code generation, testing workflows. The agent runs until task completion or max steps. LIMIT: Maximum 5 concurrent agent tasks allowed system-wide. Check running agents with list_tasks before spawning new ones.',
  inputSchema: z.object({
