...
import { createToolRegistry, createToolSearchTool, createActivateToolTool, } from '../tools/registry.js';
export async function initializeAgent(config = {}) {
    const { workspaceRoot, enableReadline = false, registry: providedRegistry, enableSemanticSearch = true, } = config;
    let rl = null;
    if (enableReadline) {
...
export async function cleanup(rl) {
